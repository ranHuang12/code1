...
def names_to_fileinfos(names):
    """
    Translate a list of GDAL filenames, into file_info entity.

    names -- list of valid GDAL dataset names.

    Returns a list of file_info entity.  There may be less file_info entity
...
class file_info(object):
    ...
    def copy_into(self, t_fh, s_band=1, t_band=1, nodata_arg=None):
        ...
        This method will compute the overlap area of the file_info entity
...
def main(argv=None):
    ...
    if names:
        names=names[0]
        dirfolder,form=names.split("*")
        names=[os.path.join(dirfolder,wcg) for wcg in os.listdir(dirfolder) if wcg.endswith(form)]
        print(len(names))
    #names是所有输入影像的列表
    if not names:

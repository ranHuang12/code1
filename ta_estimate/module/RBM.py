...
class RBM(nn.Module):
    ...
    def to_hidden_2(self, vis_prob_loader):
        """
        ...
        # Calculate hid_prob
        hid_prob_res = []
        for i, batch in enumerate(vis_prob_loader):
            d = torch.tensor(batch[0], dtype=torch.float32 , device=self.device)
            hid_prob = torch.matmul(d, self.weight)
            hid_prob = torch.add(hid_prob, self.h_bias)
            hid_prob = self.activation(hid_prob)  #计算sigmod值
            hid_prob_res.append(hid_prob)
        
        return torch.cat(hid_prob_res, dim=0).cpu().detach().numpy(), None

    def forward(self, input_data):
        return self.to_hidden_2(input_data)
    ...
    def train_rbm(self, train_dataloader, epoch=50):
        ...
        for epoch_i in range(1, epoch + 1):
            ...
            for i, batch in enumerate(train_loader):
                ...
                d = batch[0].view(( -1 , self.visible_units ))

...
def mosaic(data_list, out_path, nodata):
    ...
    for data in data_list:
        xOffset = int(round((minx_list[i] - minX) / pixelWidth))
        yOffset = int(round((maxY_list[i] - maxY) / pixelHeight))
...
def main(argv):
    ...
    reflec_path = args.reflec
    if not os.path.exists(reflec_path):
        print("输入的植被指数数据不存在，请检查！！！")
        os._exit(0)
            
    dem_path = args.dem
    if not os.path.exists(dem_path):
        print("输入的DEM数数据不存在，请检查！！！")
        os._exit(0)
    ...
    if not os.path.exists(station_file):
        print("输入的气象站点数据不存在，请检查！！！")
        os._exit(0)
        
    mete_path = args.mete
    if not os.path.exists(mete_path):
        print("输入的气象数据路径不存在，请检查！！！")
        os._exit(0)
    ...
    for hdf in os.listdir(reflec_path):
        name, ext=os.path.splitext(hdf)
        if ext!='.hdf':
           continue 
        reflectance_file = reflec_path + hdf
        index_calculte(reflectance_file)
    print ('开始镶嵌。。。')
    
    if not os.path.exists('mosaic\\LST\\'):
        os.makedirs('mosaic\\LST\\')
    mosaic_path = 'mosaic\\'
    mosaic_path_LST = mosaic_path+"LST\\"
    year_all = year_list + [year_start, year_end]
    year_min = min(year_all) 
    year_max = max(year_all)
    for lst_type in lst_type_list:
        for lst_time in lst_time_list:
            if lst_time == 0:
                time = 'day'
            else:
                time = 'night'
            for year in range(year_min,year_max+1):
                doy_all=year_days(year)
                for doy in range(1,doy_all+1):
                    LST_path_list=[]  
                    for tile in tile_list:
                        LST_file = lst_path + 'LST_source\\'+ lst_type+"."+str(year*1000+doy)+"."+tile+"."+time+".tif"
                        if not os.path.exists(LST_file):
                            break
                        LST_path_list.append(LST_file)
                        
                    for lst in LST_path_list:
                        basename = os.path.basename(lst)#去掉路径，只留下文件名
                        name_list = [basename.split('.')[0], basename.split('.')[1],'tif']
                        name = ".".join(name_list)
                        out = mosaic_path_LST + name
                    if len(LST_path_list)==0:
                        print ('%d LST数据缺失'%doy)
                    mosaic(LST_path_list, out,0)
                    print(lst + "镶嵌结束")
    
    for var in ['EVI','NDFI','NDBI']:
        mosaic_path1 = mosaic_path+var+"\\" 
        if not os.path.exists(mosaic_path1):
            os.makedirs(mosaic_path1)                   
        for year in range(year_list[0],year_list[-1]+1):
            doy_all=year_days(year)
            for doy in range(1,doy_all+1):
                path_list=[]  
                for tile in tile_list:
                    tif_file = reflec_path + var +'MCD.'+str(year*1000+doy)+"."+tile+".tif"
                    if not os.path.exists(tif_file):
                        break 
                    path_list.append(tif_file)
                for tif in path_list:
                    basename = os.path.basename(tif)#去掉路径，只留下文件名
                    name_list = [basename.split('.')[0],basename.split('.')[1],'tif']
                    name = ".".join(name_list)
                    out = mosaic_path1 + name
                if len(path_list)==0:
                    print ("%d反射率数据不存在"%doy)
                mosaic(path_list, out,-28672)
                print(tif + "镶嵌结束")

    dem_list=[]
    mosaic_path2 = mosaic_path + 'DEM\\'                
    for tile in  tile_list: 
        dem_file = dem_path + tile + "_DEM_sin_1km.tif"
        if not os.path.exists(dem_file):
            print (dem_file,"不存在")
            break
        dem_list.append(dem_file) 
    for dem in dem_list:
        basename = "DEM_sin_1km.tif" # 去掉路径，只留下文件名
        out = mosaic_path2 + basename
    mosaic(dem_list, out,32767)
    print(dem + "镶嵌结束")
                    
    print("镶嵌结束")
    
    print("开始裁剪")
    for v in ['LST', 'EVI', 'NDFI', 'NDBI', 'DEM']:
        if v=='LST':
            nodata=0
        elif v=='DEM':
            nodata=32767
        ...
        if not os.path.exists(out):
            os.makedirs(out)
        for tif_file in os.listdir(path):
            name,ext=os.path.splitext(tif_file)
            if ext!='.tif':
                continue
            clip(path+tif_file,shpfile,out+tif_file,nodata)
    print ("裁剪完毕")
    
    print ("开始使用%d-%d年的数据进行建模..."%(year_start,year_end))
    
    dem_file = "clip\\DEM\\DEM_sin_1km.tif"
    evi_path = "clip\\EVI\\"
    ndfi_path = "clip\\NDFI\\"
    ndbi_path = "clip\\NDBI\\"
    lst_path = "clip\\LST\\"
    for model_year in range(year_min,year_max+1):
        for l_type in lst_type_list:
            for lst_time in lst_time_list:
                if lst_time==0:
                    time = 'day'
                else:
                    time="night"    
                value_extract(station_file,dem_file,evi_path,ndfi_path,ndbi_path,lst_path,model_year,time,l_type)
    ...
    for l_type in lst_type_list:
        for lst_time in lst_time_list:
            if lst_time==0:
                ...
                time="night"
            LST_temp_merge(year_start,year_end,mete_path,l_type,time)

    print ("开始进行%d-%d年的气温估算"%(year_list[0],year_list[-1]))        
    for l_type in lst_type_list:
        for lst_time in lst_time_list:
            if lst_time==0:
                time = 'day'
            ...
            model_data="mete_data\\"+l_type+"_"+time+"_LST_temp.xlsx"
            if model_type == 1:
                muti_linear(year_list[0],year_list[-1],model_data,dem_file,evi_path,ndfi_path,ndbi_path,lst_path,l_type,time,out_path)
            if model_type == 2:
                RF(year_list[0],year_list[-1],model_data,dem_file,evi_path,ndfi_path,ndbi_path,lst_path,l_type,time,out_path) 
    #删除中间文件            
    shutil.rmtree("clip\\")
    shutil.rmtree(mosaic_path)
    print ("%d-%d年的气温估算完成"%(year_list[0],year_list[-1])) 

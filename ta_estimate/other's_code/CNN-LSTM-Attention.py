...
col_names = []
for name in ['lon','lat','dem','p','sm','t','sun','evi','gpp']:
...
xt_train = torch.from_numpy(x_train[:300].reshape(300, 17, 9)).float().to(DEVICE)  # 转换为张量
...
def prepare_tensor(x, y):  # 转换为张量
    return (
        torch.from_numpy(x.reshape(-1, 17, 9)).float().to(DEVICE),  # 转换为张量
        torch.from_numpy(y).float().to(DEVICE)  # 转换为张量
    )


class CNNTLSTMAttention(nn.Module):
    def __init__(self):
        super(CNNTLSTMAttention, self).__init__()
        self.conv1 = nn.Conv1d(in_channels=9, out_channels=128, kernel_size=3, stride=1, padding=1)

        self.bn1 = nn.BatchNorm1d(128)  # 添加BatchNorm1d层
        self.relu = nn.ReLU()

        self.lstm = nn.LSTM(input_size=128, hidden_size=hidden_size, num_layers=num_layers, batch_first=True)
        self.attn = nn.Linear(hidden_size + 128, 1)
    ...
    def forward(self, x):
        ...
        return fc_out.squeeze(), self.lambda_l2 * l2_loss

net = CNNTLSTMAttention() # 实例化模型
...
def prepare_tensor(x, y): # 转换为张量
    return (
        torch.from_numpy(x.reshape(-1, 9, 1)).float().to(DEVICE), # 转换为张量
        torch.from_numpy(y).float().to(DEVICE) # 转换为张量
    )
...
loss_train_list, rmse_train_list, rmse_val_list, r2_val_list,r2_train_list,mare_val_list = [], [], [], [],[],[] # 初始化列表

for epoch in range(1, 1900): # 遍历训练轮数
    net.train() # 模型转换为训练模式
    ...
    mare_val_list.append(mare_val)

    print('[epoch {:d}] mare_val: {:.4f}, training rmse: {:.4f}, val rmse: {:.4f}, val R^2: {:.4f}, train R^2: {:.4f}'.format(epoch, mare_val, rmse_train, rmse_val, r2_val, r2_train)) # 修改此处，添加训练集 R^2

    if (r2_val>0.4): # 如果测试集RMSE小于等于1.008
        # 使用StandardScaler进行反归一化
        wanted_pred_t = yield_scaler.inverse_transform(y_val_pred.reshape(-1, 1)).flatten() # 保存验证集预测值
        torch.save(net.state_dict(), 'model.pth') # 保存模型参数

model=CNNTLSTMAttention()
...
xt_test = torch.from_numpy(x_test.reshape(-1, 17, 9)).float().to(DEVICE)  # 转换为张量
...
pd.DataFrame(wanted_pred_t).to_csv('2017CNN-LSTM-A.csv', header=None, index=None)
